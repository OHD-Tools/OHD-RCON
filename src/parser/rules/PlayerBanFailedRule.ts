...
const PlayerBanFailedRule: LogParserRule<PlayerBanned> = {
  regex: /^Failed to ban specified player by (?<type>name|id number) with reason '(?<reason>.*)', no player with the (name|id number) '(?<identifier>.*)' exists or the ban operation failed!/,
  format: (oargs) => {
